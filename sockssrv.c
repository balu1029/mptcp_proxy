...
static int connect_socks_target(unsigned char *buf, size_t n, struct client *client) {
	...
	printf("Port: %d, remote: %d",port,&remote);
	...
	//change mptcp values according to file
	int mptcpEnable = 0;
	...
	if(strcmp(ip,namebuf) == 0)					// did a entry in config file match to destination?
	{
		...
		else if(strcmp(ptr,"roundrobin") == 0)
		{
			char scheduler[] = "roundrobin";
		}
		else if(strcmp(ptr,"redundant") == 0)
		{
		...
		else if(strcmp(ptr,"fullmesh") == 0)
		{
			char pathManager[] = "fullmesh";
		}
		else if(strcmp(ptr,"ndiffports") == 0)
		{
			char pathManager[] = "ndiffports";
		}
		else if(strcmp(ptr,"binder") == 0)
		{
	...
	if(SOCKADDR_UNION_AF(&bind_addr) != AF_UNSPEC && bindtoip(fd, &bind_addr) == -1)
		goto eval_errno;
	printf("address: %d", remote->ai_addr);
